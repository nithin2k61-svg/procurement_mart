...
WITH stg_crev_aggr as (
    select 
    a.client as CLIENT_ID,
    --product as PRODUCT_TYPE_ID,
    case when trim(lower(a.product)) in('prd0028 vcc fax','prd0030 vcc download') and lower(account_code) like '%unsettled%' then concat(trim(a.product), '-',trim(a.account_name))
   ...
   'ACTUALS' as TRAN_TYP,
    coalesce(sum(rollup),0) as CREV_AMOUNT,
...
select * from stg_crev_aggr  order by  PRODUCT_TYPE_ID,BUSINESS_UNIT_ID,DATEKEY  
