...
with intr_fact_revenue_adaptive_aggr as (
select
...
z.REVENUE_GL_ACCOUNT as REVENUE_GL_ACCOUNT,
coalesce(sum(REV_AMOUNT),0) as REV_AMOUNT,
...
from {{ref('stg_revenue_adaptive_aggr')}} z
...
REV_AMOUNT,
SRC_SYS_ID,
concat(dim_client_id,'_',dim_product_type_id,'_',dim_source_system_id,'_',dim_business_unit_id,'_',dim_date_id,'_',dim_provider_id,ACCOUNT_NAME,LEVEL_NAME,TRAN_TYP,REVENUE_GL_ACCOUNT) as src_uniq_cd,
ROW_CRE_DT,
ROW_CRE_USR_ID,
ROW_MOD_DT,
ROW_MOD_USR_ID

from intr_fact_revenue_adaptive_aggr
)
select * from final
